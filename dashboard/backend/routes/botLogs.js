...
const LOG_CANDIDATES = [
  ...
  '/root/.pm2/logs/ZiSi-Core-Engine-out.log',
  '/root/.pm2/logs/ZiSi-Core-Engine-error.log',
];
...
router.get('/', (req, res) => {
  ...
  if (fileParam === 'positions') {
    candidates = [path.join(BOT_ROOT, 'data', 'positions_state.json')];
  } else if (fileParam === 'account') {
    candidates = [path.join(BOT_ROOT, 'data', 'account_state.json')];
  } else if (fileParam === 'signals') {
    candidates = [path.join(BOT_ROOT, 'data', 'signal_evaluations.jsonl')];
  } else if (fileParam === 'gates') {
    candidates = [path.join(BOT_ROOT, 'data', 'gate_log.jsonl')];
  }
...
router.get('/sizes', (req, res) => {
  const LOG_FILES = {
    ...
    'PM2 Engine Out':     '/root/.pm2/logs/ZiSi-Core-Engine-out.log',
    'PM2 Engine Error':   '/root/.pm2/logs/ZiSi-Core-Engine-error.log',
    'Signal Evaluations': path.join(BOT_ROOT, 'data', 'signal_evaluations.jsonl'),
    'Gate Log':           path.join(BOT_ROOT, 'data', 'gate_log.jsonl'),
  };
  const files = {};
  let totalBytes = 0;
...
router.post('/clear', (req, res) => {
  try {
    ...
    // Trigger PM2 flush to empty active PM2 stdout/stderr files on VPS
    exec('pm2 flush', (err, stdout, stderr) => {
      if (err) {
        console.error('[LOGS] PM2 flush failed:', err.message);
      } else {
        console.log('[LOGS] PM2 flush executed successfully:', stdout.trim());
      }
    });

    res.json({ status: 'success', message: 'Logs cleared successfully and PM2 log files flushed', cleared: clearedPaths });
  } catch (err) {
