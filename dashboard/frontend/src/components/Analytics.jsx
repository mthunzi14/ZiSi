...
function parsePillarAndType(title, entryType) {
  const type = parseType(title, entryType);
  let pillar = 'CORE_SNIPER';
  if (type === 'LAT ARB' || type === 'LAT_ARB') {
    pillar = 'LATENCY_ARBITRAGE';
  } else if (type === 'SWEEP' || type === 'NCS' || type === 'REV STREAK' || type === 'REV_STREAK') {
...
function buildPillarStats(closed) {
  const pillars = {
    CORE_SNIPER: { name: 'Core Sniper (Pillar 1)', wins: 0, total: 0, pnl: 0 },
    ASYMMETRIC_BARBELL: { name: 'Asymmetric Barbell (Pillar 2)', wins: 0, total: 0, pnl: 0 },
    LATENCY_ARBITRAGE: { name: 'Latency Arbitrage (Pillar 3)', wins: 0, total: 0, pnl: 0 }
  };
  const list = closed || [];
  for (const t of list) {
    const { pillar } = parsePillarAndType(t.event_title, t.entry_type || t.trade_type);
    if (pillars[pillar]) {
      pillars[pillar].total += 1;
      pillars[pillar].pnl += parseFloat(t.realized_pnl || 0);
      if (parseFloat(t.realized_pnl || 0) > 0) {
        pillars[pillar].wins += 1;
      }
    }
  }
  return Object.entries(pillars).map(([id, d]) => ({
    id,
    name: d.name,
    wr: d.total > 0 ? Math.round((d.wins / d.total) * 100) : 0,
    pnl: Math.round(d.pnl * 100) / 100,
...
function buildPillarAllocation(active) {
  const pillars = {
    CORE_SNIPER: { size: 0 },
    ASYMMETRIC_BARBELL: { size: 0 },
    LATENCY_ARBITRAGE: { size: 0 }
  };
  let totalActiveSize = 0;
  const list = active || [];
  for (const t of list) {
    const { pillar } = parsePillarAndType(t.event_title, t.entry_type || t.trade_type);
    const sizeVal = parseFloat(t.size || 0);
    if (pillars[pillar]) {
      pillars[pillar].size += sizeVal;
    }
    totalActiveSize += sizeVal;
  }
  return Object.fromEntries(
    Object.entries(pillars).map(([id, d]) => [
      id,
      {
        size: Math.round(d.size * 100) / 100,
        pct: totalActiveSize > 0 ? Math.round((d.size / totalActiveSize) * 100) : 0
      }
    ])
  );
}
...
const PILLAR_META = {
  CORE_SNIPER: { name: 'Core Sniper (Pillar 1)', color: '#00d4a3', desc: 'Quarter-Kelly selective trading' },
  ASYMMETRIC_BARBELL: { name: 'Asymmetric Barbell (Pillar 2)', color: '#f59e0b', desc: 'Flat 1% or Half-Kelly' },
  LATENCY_ARBITRAGE: { name: 'Latency Arbitrage (Pillar 3)', color: '#2b7fff', desc: 'Full Kelly * beta latency' }
};
...
const Analytics = memo(function Analytics({ active = [], closed = [] }) {
  const safeActive = active || [];
  const safeClosed = closed || [];
  const assetStats    = useMemo(() => buildAssetStats(safeClosed), [safeClosed]);
  const exitBreakdown = useMemo(() => buildExitBreakdown(safeClosed), [safeClosed]);
  const volumeSeries  = useMemo(() => buildVolumeSeries(safeClosed), [safeClosed]);
  const hourlyPnl     = useMemo(() => buildHourlyPnl(safeClosed), [safeClosed]);
  const runningEv     = useMemo(() => buildRunningEv(safeClosed), [safeClosed]);
  const typeStats     = useMemo(() => buildTypeStats(safeClosed), [safeClosed]);
  const pillarStats   = useMemo(() => buildPillarStats(safeClosed), [safeClosed]);
  const pillarAllocation = useMemo(() => buildPillarAllocation(safeActive), [safeActive]);

  const totalTrades = safeClosed.length;
  const totalPnl    = useMemo(() => {
    return Math.round(safeClosed.reduce((s, t) => s + parseFloat(t.realized_pnl || 0), 0) * 100) / 100;
  }, [safeClosed]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }} className="page-fade-enter">
      ...
      {/* Strategy Pillar Performance overview card */}
      <div className="card" style={{ padding: '20px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <div style={S.title}>Performance by Strategy Pillar</div>
            <div style={S.sub}>Real-time telemetry and capital allocation across trading pillars</div>
          </div>
        </div>
        
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          {pillarStats.map(({ id, name, wr, pnl, count }) => {
            const meta = PILLAR_META[id] || { name, color: '#888', desc: '' };
            const alloc = pillarAllocation[id] || { size: 0, pct: 0 };
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h4 style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: 'var(--color-obsidian)' }}>{meta.name}</h4>
                    <span style={{ fontSize: '10px', color: 'var(--color-iron)' }}>{meta.desc}</span>
                  </div>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 600, background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '4px', color: 'var(--color-iron)' }}>
                    {count} trades
                  </span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '4px' }}>
                  <div>
                    <div style={{ fontSize: '9px', color: 'var(--color-iron)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Win Rate</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', fontWeight: 700, color: wr >= 65 ? 'var(--color-profit)' : wr >= 50 ? '#f59e0b' : 'var(--color-loss)' }}>
                        {wr}%
                      </span>
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '9px', color: 'var(--color-iron)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total Yield</div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', fontWeight: 700, color: pnlColor, marginTop: '2px' }}>
                      {pnl > 0 ? '+' : ''}${pnl.toFixed(2)}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ height: '4px', background: 'rgba(255,255,255,0.04)', borderRadius: '2px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${wr}%`, background: wr >= 65 ? 'var(--color-profit)' : wr >= 50 ? '#f59e0b' : 'var(--color-loss)', borderRadius: '2px' }} />
                </div>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '10px', color: 'var(--color-iron)' }}>Active Allocation:</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', fontWeight: 700, color: 'var(--color-obsidian)' }}>
                    ${alloc.size.toFixed(2)} <span style={{ color: 'var(--color-iron)', fontWeight: 400 }}>({alloc.pct}%)</span>
                  </span>
                </div>
              </div>
            );
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              background: collapsed ? 'rgba(109, 129, 161,0.1)' : 'rgba(255,255,255,0.04)',
              border: `1px solid ${collapsed ? 'rgba(109, 129, 161,0.35)' : 'rgba(255,255,255,0.08)'}`,
              borderRadius: 8, padding: '5px 11px', cursor: 'pointer',
              fontSize: 10, fontWeight: 700,
              color: collapsed ? '#6d81a1' : '#a1a1aa',
              transition: 'all 0.2s',
            }}
          >
