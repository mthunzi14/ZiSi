...
/**
 * GhostCursor component creates a fluid, smoky cursor trail effect using Three.js WebGL.
 * It features custom fragment shaders for noise and organic movements, combined with
 * UnrealBloom and FilmGrain post-processing filters.
 */
export const GhostCursor = ({
  ...
  bloomThreshold = 0.01,
  brightness = 1.3,
  color = '#bbdef2', // MidnightCommand iridescent glow token default
  mixBlendMode = 'screen',
  edgeIntensity = 0,
  ...
  const pixelBudget = targetPixels ?? (isTouch ? 0.8e6 : 1.2e6);
  ...
  const fragmentShader = `
    ...
    vec3 tint2(vec3 base){ return mix(base, vec3(0.85, 0.92, 1.0), 0.2); }
    ...
    void main() {
      ...
      float fillMask = mix(1.0 - k, 1.0, distFromEdge);
      
      float outAlpha = clamp(alphaAcc * iOpacity * fillMask, 0.0, 1.0);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (trailBufRef.current[headRef.current]) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        if (arr[i] && trailBufRef.current[srcIdx]) {
