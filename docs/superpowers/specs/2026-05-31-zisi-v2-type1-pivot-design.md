...
### 10.11 Key & wallet security (LIVE phase only — zero risk in demo)
PBot's explicit warning: a single API-key/wallet mistake = wallet drained, bot gone, months erased. **In demo there is no exposure** (no real keys, no real orders). When we reach the **live** stage, enforce: (1) a **dedicated trading wallet** funded with *only* the trading bankroll, never the main wallet; (2) API keys scoped to **order placement only** (no withdrawal perms) where Polymarket supports it; (3) keys **only** in a `chmod 600 .env` on the VPS, never in git, chats, or screenshots — **rotate any key ever shared**; (4) the VPS firewalled (only SSH + nginx 80/443; dashboard behind HTTP Basic Auth). The detailed runbook already lives in `docs/VPS_MIGRATION.md`; this is the security gate that must be satisfied before any live capital.
