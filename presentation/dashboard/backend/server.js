...
function startBot() {
  ...
  console.log('');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('🚀  Spawning ZiSi Bot  (python app/main.py)');
...
function startHeartbeatWatchdog() {
  ...
  watchdogInterval = setInterval(() => {
    if (botStopped || !botProcess) return;
    
    const accountStatePath = path.join(BOT_ROOT, 'account_state.json');
    if (!fs.existsSync(accountStatePath)) return;
    
    try {
      const data = JSON.parse(fs.readFileSync(accountStatePath, 'utf8'));
      if (data.last_updated) {
        ...
        // If last updated is older than 4 minutes, bot is stale!
        if (diffMins > 4.0) {
          console.log(`\n🚨  [WATCHDOG] Bot heartbeat is stale (${diffMins.toFixed(1)} minutes old). Triggering self-healing restart...`);
          // Kill the process forcefully so the unexpected exit handler triggers restart
          if (process.platform === 'win32') {
            try {
              execSync(`taskkill /F /T /PID ${botProcess.pid}`);
            } catch (e) {
